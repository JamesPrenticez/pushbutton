...
export default function Notes() {
  ...
  async function get() {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/notes`, {
        method: "GET",
        headers: {
    ...
    <>
      ...
      <div className="flex justify-center">
        <main className="w-full max-w-3xl xl:max-w-7xl space-y-4">
          ...
          <div className="bg-[#21262b] cursor-pointer p-4 text-white">
            <div className="space-y-2">
              <div className="flex justify-between">
                <h1 className="text-ocean-1 text-2xl">Get from Python API</h1>
                <div className="ml-auto">
                  <button className="bg-[#007FFF] p-4" onClick={() => get()}>
                    GET
                  </button>
                </div>
              </div>
